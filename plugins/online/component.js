...
function component(object){
    ...
    this.find = function(){
        ...
        if(object.movie.imdb_id){
            ...
            network.native('https://'+(Lampa.Storage.field('proxy_tmdb') === false ? 'api.themoviedb.org' : 'apitmdb.cub.watch')+'/3/' + (object.movie.name ? 'tv' : 'movie') + '/' + object.movie.id + '/external_ids?api_key=4ef0d7355d9ffb5151e987764708ce96&language=ru', function (ttid) {
                letgo(ttid.imdb_id)
            },(a, c)=>{
                this.empty(network.errorDecode(a,c))
            })
