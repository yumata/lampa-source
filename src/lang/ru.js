export default {
    ...
    termsofuse_t_04: 'Lampa использует API (application programming interface) от каталога themoviedb.org для получения информации о фильмах и телесериалах, а также для просмотра связанных с ними медиа-файлов, таких как постеры, трейлеры и другое.',
    termsofuse_t_05: 'API от themoviedb.org позволяет приложению Lampa получать доступ к базе данных фильмов и телесериалов, предоставляя возможность поиска и просмотра информации медиа-файлов.',
    termsofuse_t_06: 'Взаимодействие между приложением Lampa и themoviedb.org происходит через API запросы, которые отправляются с приложения Lampa на сервера themoviedb.org для получения информации и медиа-файлов. Themoviedb.org обрабатывает запросы и отправляет обратно на приложение Lampa запрошенные данные.',
    ...
    termsofuse_t_12: 'Если вы обнаружили нарушение авторских прав в приложение Lampa, то вам следует связаться с разработчиками приложения Lampa. Для этого вы можете воспользоваться контактами в приложение Lampa. Однако, важно учесть, что приложение Lampa использует API от каталога themoviedb.org, который предоставляет открытый доступ к информации о фильмах и сериалах. Это означает, что информация, которая отображается в приложения Lampa, является общедоступной и не нарушает авторских прав.',
    termsofuse_t_13: 'Если вы считаете, что в приложения Lampa все же есть контент, который нарушает ваши авторские права, вы можете обратиться к правообладателю этого контента и попросить его удалить со страниц сайта themoviedb.org. В свою очередь, приложение Lampa может удалить контент при получении соответствующего уведомления о нарушении авторских прав.',
}
